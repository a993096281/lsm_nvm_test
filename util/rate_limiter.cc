...
int64_t GenericRateLimiter::CalculateRefillBytesPerPeriod(
  ...
  if (kMaxInt64 / rate_bytes_per_sec < refill_period_us_) {
    // Avoid unexpected result in the overflow case. The result now is still
    // inaccurate but is a number that is large enough.
    return kMaxInt64 / 1000000;
  } else {
...
Status GenericRateLimiter::Tune() {
  ...
  assert(num_drains_ - prev_num_drains_ <= kMaxInt64 / 100);
  ...
  if (drained_pct == 0) {
    ...
    int64_t sanitized_prev_bytes_per_sec =
        std::min(prev_bytes_per_sec, kMaxInt64 / 100);
    new_bytes_per_sec =
    ...
    int64_t sanitized_prev_bytes_per_sec = std::min(
        prev_bytes_per_sec, kMaxInt64 / (100 + kAdjustFactorPct));
    new_bytes_per_sec =
